...
app.post("/api/solve", async (req, res) => {
  ...
  try {
    ...
    const messages = [
      {
        role: "system",
        content: "你是有机化学助教，回答要详细、有条理。如果相关信息在下面的检索到的知识片段中，请优先引用并标注来源编号（例如 [1], [2]）。"
      },
    ...
    res.json({
      ...
      query: question,
      text: answerText,
      sources: scored.map((s, i) => ({ idx: i + 1, snippet: s.content.slice(0, 400), score: s.score, doc_id: s.doc_id }))
    });
  } catch (err) {
    console.error("DeepSeek RAG error:", err.response?.data || err.message || err);
    res.status(500).json({
      error: true,
